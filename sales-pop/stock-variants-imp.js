    ...
    window.checkmodule_popup = function (response) {
        ...
        if (apiResponse && apiResponse.stock && apiResponse.stock !== null) {
            if (apiResponse.stock.on_off == 1)
            {
                ...
                } else if (stock_restriction_setting.stock_restriction_check && stock_restriction_setting.stock_restriction_check == "on" && apiResponse.stock.variantCheck == 1 && apiResponse.stock.variantCheck.length == 1 && parseInt(apiResponse.stock.left_stock) > parseInt(stock_restriction_setting.stock_restriction_value)) {
                    console.log("SP: Stock not going to display due to restriction");
                } else {
                    $jq321("head").append($jq321("<link/>", {
                        rel: "stylesheet",
                        href: serverUrl.cssStock + "?v" + version
