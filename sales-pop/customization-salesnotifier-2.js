    ...
    function stockCountdown(responseStock) {
        ...
        if (responseStock.above_cart == 1) {
            if (masterSelector.length > 0) {
                $jq321(responseStock.view).insertBefore(masterSelector);
                if (responseStock.soldOutSettings !== undefined && responseStock.soldOutProduct === undefined) {
                    $jq321(responseStock.soldView).insertBefore(finalSelector);
                }
            }
            ...
            if (masterSelector.length > 0) {
                ...
                if (responseStock.soldOutSettings !== undefined && responseStock.soldOutProduct === undefined) {
                    $jq321(responseStock.soldView).insertAfter(finalSelector);
                }
            }
            else if (selectorStock1.length == 1) {
